"""engine/providers/deepseek_agent.py — CC SDK + DeepSeek API 后端。

复用 claude-agent-sdk 的 agent loop（tool-use、session 管理），
但通过 env 把 ANTHROPIC_BASE_URL / ANTHROPIC_AUTH_TOKEN 指向 DeepSeek 端点。
适合需要 tool-use 但想用 DeepSeek 模型的编码任务。
"""
...
def _looks_like_api_failure(text: str) -> bool:
    lowered = (text or "").lower()
    return any(tag in lowered for tag in _API_FAIL_MARKERS)
...
class DeepSeekAgentProvider(Provider):
    """CC SDK agent loop，后端换成 DeepSeek API（需要 DEEPSEEK_API_KEY）。"""
    name = "deepseek"
    ...
    def __init__(self, store: SessionStore, api_key: str, base_url: str):
        ...
        self._env = {
            ...
            # 关掉每请求变化的 cch 字段，保住 DeepSeek 侧 prompt 缓存命中率
    ...
    async def run(self, *, prompt, model, cwd=".", env=None, session_id=None,
        ...
        if not self._has_key:
            return AgentResult(text="", ok=False,
                               error="DeepSeekAgentProvider: DEEPSEEK_API_KEY 未设置")
        try:
            ...
            async with sess.lock:
                ...
                if text and not text.endswith("\n"):
                    print()
            # CC SDK 遇到认证/API失败时可能不抛异常，把错误放进 result text.
            # Keep these markers specific; normal plans often contain "400px" or
            # localhost:3000 and must not be treated as provider failures.
            if _looks_like_api_failure(text):
                return AgentResult(text="", session_id=None, ok=False,
                                   error=f"DeepSeek API 错误（触发 fallback）: {text[:200]}")
